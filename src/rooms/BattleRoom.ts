...
export class BattleRoom extends Room<BattleState> {
  ...
  onCreate(options: any) {
    ...
    if (options.fromLobby && options.preReadyPlayers) {
      ...
      this.clock.setTimeout(() => {
        this.startGame()
      }, 2000) // 2 second delay for all players to connect
    }

    console.log(`🎮 BattleRoom ${this.roomId} created and ready for battle`)
  }
  ...
  async onAuth(client: Client, options: any) {
    ...
    try {
      ...
      console.error(
        `❌ BattleRoom: Authentication failed for ${client.sessionId}:`,
  ...
  onLeave(client: Client, consented: boolean) {
    console.log(`👋 Player ${client.sessionId} left the battle room (consented: ${consented})`)
